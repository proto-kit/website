...
When the `LocalTaskWorkerModule` is started it has a subset of start-up tasks, like those above, passed to it. These tasks basically
...
1. The worker registers the callbacks for `Unprepared` tasks with the queue, one of which is the
`WorkerRegistration` task. This is what receives the worker payload.
...
3. The `SequencerStartupModule` invokes  the `WorkerRegistrationFlow`, which submits a `WorkerRegistration` task in a boundless
loop to the queue.
4. The worker listens to the queue, processes the `WorkerRegistration` task and is initialised.
5. The AppChain continues and starts submitting tasks, like BlockProving and StateTransition, say, to the queue.
6. The worker calls the `prepare()` method for the `Regular` tasks.
7. The worker registers `Regular` tasks with the queue.
8. The worker is started and able to process tasks emitted to the queue.
