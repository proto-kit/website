...
6. The AppChain continues and starts submitting tasks, like BlockProving and StateTransition, say, to the queue.
7. The worker calls the `prepare()` method for the `Regular` tasks.
8. The worker registers `Regular` tasks with the queue.
9. The worker is now able to process all configured tasks emitted to the queue.
