...
```ts {12,17,19} showLineNumbers filename="ProtocolStateModule.ts" copy file=<rootDir>/starter-kit-snippets/packages/chain/src/runtime/hooks/transactionHook.ts
...
```typescript filename="validNetworkStateHashBlockHook.ts" copy file=<rootDir>/starter-kit-snippets/packages/chain/src/runtime/hooks/validNetworkStateHashBlockHook.ts
```
