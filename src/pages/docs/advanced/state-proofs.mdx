...
In case you want to learn more about the state related behind-the-scenes, check out [Foundational Concepts/State Model](/docs/concepts/state-model)
...
```graphql filename="GraphQL" copy
query UseState {
  runtime {
    testModule {
      userState(key: "address") {
        property1
        property2
      }
    }
