...
export class Balances extends RuntimeModule<BalancesConfig> {
  ...
  public hasBalance(address: PublicKey): Bool {
    return (await this.balances.get(address)).isSome;
  }
}
```
...
export class Balances extends RuntimeModule<BalancesConfig> {
  ...
  public getBalance(address: PublicKey): UInt64 {
    const balance = (await this.balances.get(address)).orElse(UInt64.from(0));
    return UInt64.from(balance.value);
  }
}
```
...
class MyModule extends RuntimeModule<Record<never, never>> {
  ...
  @runtimeMethod()
  public async hashStruct() {
    const myStruct = new MyStruct((await this.myStruct.get()).value);
...
export class Balances extends RuntimeModule<BalancesConfig> {
  ...
  @runtimeMethod()
  public async setBalance(address: PublicKey, amount: UInt64) {
