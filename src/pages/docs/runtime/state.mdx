...
state map is a mapping between two values (key-value). You can have an idefinite number of state properties on a module,
...
```typescript {15-18, 20} showLineNumbers filename="balances.ts"
import {
  RuntimeModule,
  runtimeModule,
  state,
...
```typescript {22-24, 26-28} showLineNumbers filename="balances.ts"
import {
  RuntimeModule,
  runtimeModule,
  state,
...
export class Balances extends RuntimeModule<BalancesConfig> {
  ...
  public getCirculatingSupply(): Option<UInt64> {
    return this.circulatingSupply.get();
  }
}
```
...
export class Balances extends RuntimeModule<BalancesConfig> {
  ...
  public getBalance(address: PublicKey): UInt64 {
    return this.balances.get(address).orElse(UInt64.from(0));
  }
}
```
...
export class Balances extends RuntimeModule<BalancesConfig> {
  ...
  public setBalance(address: PublicKey, amount: UIn64) {
