...
export class Balances extends RuntimeModule<BalancesConfig> {
  ...
  public mint(address: PublicKey, amount: UInt64) {
    assert(this.network.height.lessThanEqual(UInt64.from(1000)), "Minting ended at block height #1000");
...
export class Balances extends RuntimeModule<BalancesConfig> {
  ...
  public mint(amount: UInt64) {
    assert(this.transaction.nonce.equals(0), "Only new users can mint");
