...
export class Balances extends RuntimeModule<BalancesConfig> {
  ...
  public async mint(amount: UInt64) {
    ...
    await this.balances.set(this.transaction.sender, amount);
  }
}
...
export class CustomBalances extends Balances {
  ...
  public override async mint() {
    await this.balances.set(this.transaction.sender, UInt64.from(1));
  }
}
...
export class Vesting extends RuntimeModule<Record<string, never>> {
  ...
  @runtimeMethod()
  public async claim() {
    await this.balances.mint(this.transaction.sender, UInt64.from(1000))
  }
}
```