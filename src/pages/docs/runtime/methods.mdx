...
```typescript {19-22} showLineNumbers filename="balances.ts"
import {
  RuntimeModule,
  runtimeModule,
  state,
...
export class Balances extends RuntimeModule<Record<string, never>> {
  ...
  @runtimeMethod()
  public async mint(canMintProof: CanMintProof, amount: UInt64) {
    await canMintProof.verify();
    await this.balances.set(address, amount);
  }
}
```
...
```typescript {19-25} showLineNumbers filename="balances.ts"
import {
  RuntimeModule,
  runtimeModule,
  state,
...
export class Balances extends RuntimeModule<BalancesConfig> {
  ...
  public async transfer(from: PublicKey, to: PublicKey, amount: UInt64) {
    const { value: fromBalance } = await this.balances.get(from);
