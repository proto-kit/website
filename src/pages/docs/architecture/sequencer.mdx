...
Following the modularity principles, all working components of the sequencer are _sequencer modules_, and may be added, removed or replaced at developer's convenience.
...
state from one transaction to another. This allows us to circumvent any potential race conditions stemming from client side parallelism, as it may be the case for the Mina L1 smart contracts.
...
The aforementioned architecture allows us to parallelize the most resource intensive part of the block production process, which is the proving itself.
